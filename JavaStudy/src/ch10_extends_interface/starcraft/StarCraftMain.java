...
//		protossList.add(highTemplar);
		...
		ArrayList<StarUnit> starList = new ArrayList<>();  // 부모객체로 리스트만듬
...
//		starList.get(4).tthunderStrom();
		...
		// 하이템플러 -> 스타유닛이 -> 하이템플러 (o)
		// 하이템플러 -> 스타유닛 -> 질럿,마린 (x)
//		((HighTemplar)starList.get(0)).move(5,5);    starList.get(0) =질럿  질럿->스타유닛 ->하이템플러 (x)
		...
		// 자식객체는 부모에서 확장된 거기때문에 부모 ->자식으로 불가.
