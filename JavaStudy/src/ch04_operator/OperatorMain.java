...
public class OperatorMain {
	...
	public static void main(String[] args) {
		...
		// 홀수, 짝수 판별
		int anyNum = 5437;
		// anyNum을 2로 나눈 나머지가 1이면 홀수 , 0 이면 짝수
		System.out.println(anyNum % 2);
		
		// 게시판 페이징 구현
		int totalCountGul = 33;  // 게시판에 글이 총 33개 있다.
		int showCountOnePage = 10; // 한 페이지에서 보여줄 글의 수가 10
		
		// 총 페이지의 수
		// 33 / 10 = 3.3 -> 올림처리 4 .0 -> 4  
		int countPage = (int)Math.ceil((double)totalCountGul / showCountOnePage);   //Math.ceil 올림처리
...
//														
		// inputId 의 길이가 7이상이면 통과, 아니면 재입력    () ? () : ()
		String check = (inputId.length() >=7 ) ? ("통과") : ("재입력");
		System.out.println(check);
		
		
		// inputId 가 empty가 아니라면 1, empty가 맞다면 0
		...
		// &&
		// 좌우 조건이 전부 true 이어야만 true 리턴
		//좌우 둘중 하나라도 false 가 있다면 false 리턴
		System.out.println(inputAge >= 14 && inputPhone.length() == 11 );
		
		
		// 휴대폰 번호는 10자리 혹은 11자리 여야한다.
		// 10 <= x <= 11
...
//		System.out.println(10<= inputPhone.length() <=11);   
		...
		// || 
		// 좌우 조건중 하나라도 true 면 true리턴   
	 	// 둘다 false 일때만  false 리턴 
...
//						^  // 0011 XOR 연산 = 3
		...
		// 같은 자리에 숫자가 둘 다 1이어야 1로 계산한다.
		System.out.println(bitTen & bitNine);
		
		// | OR
		// 이진수 기준으로 한자리라도 1이면 1로계산한다.
		System.out.println(bitTen | bitNine);
		
		// ^ XOR
		// 이진수 기준으로 각자리가 서로 다른경우에만 1로계산
		System.out.println(bitTen ^ bitNine);
